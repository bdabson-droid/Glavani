...
def build_home_hero_videos() -> None:
    """Build poster/WebM from uploaded hero MP4s. Never overwrites existing MP4 files."""
    if not shutil.which("ffmpeg"):
        print("  warn: ffmpeg not found — skipping home hero video assets")
        return
    img_dir = ROOT / "images"
    pairs = (
        (HOME_LANDING_VIDEO_PORTRAIT, HOME_LANDING_GIF_PORTRAIT),
        (HOME_LANDING_VIDEO_LANDSCAPE, HOME_LANDING_GIF_LANDSCAPE),
    )
    for base_name, gif_name in pairs:
        ...
        if not mp4_path.is_file() and gif_path.is_file():
            subprocess.run(
                [
            ...
            if mp4_path.is_file():
                print(f"  image: {mp4_path.name} (from GIF fallback)")

        if not mp4_path.is_file():
            continue

        if _needs_rebuild(poster_path, mp4_path):
            subprocess.run(
                [
                    "ffmpeg", "-y", "-ss", "0", "-i", str(mp4_path),
        ...
        if _needs_rebuild(webm_path, mp4_path):
            subprocess.run(
                [
                    "ffmpeg", "-y", "-i", str(mp4_path),
                    "-c:v", "libvpx-vp9", "-crf", "30", "-b:v", "0", "-row-mt", "1", "-an",
                    str(webm_path),
                ],
                check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
            )
...
def resolve_home_landing_media() -> dict[str, dict[str, str]]:
    """Return portrait/landscape hero media paths (uploaded MP4 preferred)."""
    img_dir = ROOT / "images"
    fallback = f"/images/{HOME_LANDING_FALLBACK}"

    def pick(base_name: str, gif_name: str, *, width: int, height: int) -> dict[str, str]:
        ...
        if mp4.is_file() and poster.is_file():
            ...
            if webm.is_file() and webm.stat().st_size < mp4.stat().st_size:
                item["webm"] = f"/images/{quote(webm.name)}"
            return item
        if (img_dir / gif_name).is_file():
            return {
                "poster": f"/images/{quote(gif_name)}",
                "mode": "image",
                "width": str(width),
                "height": str(height),
            }
    ...
    return {
        "portrait": pick(
            HOME_LANDING_VIDEO_PORTRAIT, HOME_LANDING_GIF_PORTRAIT, width=1080, height=1920
        ),
        "landscape": pick(
            HOME_LANDING_VIDEO_LANDSCAPE, HOME_LANDING_GIF_LANDSCAPE, width=1920, height=1080
        ),
    }
...
def render_home_hero_background(media: dict[str, dict[str, str]]) -> str:
    ...
    if landscape["mode"] == "video":
        poster = (
            f'<img class="site-header__hero-poster" src="{landscape["poster"]}" alt="" '
            f'width="{landscape["width"]}" height="{landscape["height"]}" '
            f'{img_loading_attrs(lcp=True)} aria-hidden="true">'
        )
        return poster + render_home_hero_video(media, orientation="landscape")
    return render_home_hero_media(media, orientation="landscape")
...
@media(max-width:899px){.site-header__hero-poster,.site-header__hero-media{height:calc(100% / .88);min-height:calc(100% / .88);min-width:calc(100% / .88);transform:translate(-50%,-50%) scale(.88)}.site-header--home-video .site-header__brand{top:clamp(2.25rem,9vh,4.5rem);width:min(76vw,320px)}.site-header__tagline--landing{bottom:clamp(3.25rem,12vh,5.5rem);width:min(88vw,36rem);padding:.9rem 1.25rem 1rem;font-size:clamp(1.05rem,3.6vw,1.45rem)}}
...
.site-header--home-video .site-header__brand{position:absolute;top:clamp(2.75rem,11vh,5.25rem);left:50%;transform:translateX(-50%);width:min(80vw,360px);max-width:none;margin:0}
.site-header__tagline--landing{position:absolute;left:50%;bottom:clamp(4.75rem,19vh,8rem);transform:translateX(-50%);width:min(90vw,40rem);margin:0;padding:1rem 1.4rem 1.1rem;max-width:none;font-family:Georgia,"Times New Roman",serif;font-size:clamp(1.2rem,4vw,1.85rem);line-height:1.35;font-weight:700;font-style:italic;letter-spacing:.04em;text-align:center;color:#fff;background:rgba(0,0,0,.2);border:2px solid rgba(245,166,35,.75);border-radius:4px 20px 4px 20px;box-shadow:0 8px 28px rgba(0,0,0,.35);text-shadow:0 2px 12px rgba(0,0,0,.95),0 0 24px rgba(0,0,0,.65);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);quotes:none}
.site-header__lang--corner{position:absolute;top:max(1rem,env(safe-area-inset-top,0px));right:max(1rem,env(safe-area-inset-right,0px));z-index:3;display:inline-flex;align-items:center;justify-content:center;min-width:2.625rem;min-height:2.625rem;padding:.525rem .64rem;border-radius:8px;border:2px solid #f58220;background:#f58220;color:#fff;text-decoration:none;font-size:.75rem;font-weight:800;letter-spacing:.04em;line-height:1;box-shadow:0 6px 20px rgba(0,0,0,.35)}
.site-header__lang--corner:hover{background:#d96a12;border-color:#d96a12;color:#fff;text-decoration:none}
...
def render_home_hero_preload() -> str:
    ...
    landscape = media["landscape"]
    return f'  <link rel="preload" href="{landscape["poster"]}" as="image" fetchpriority="high">'
...
def site_header(lang: str, is_home: bool = False) -> str:
  ...
  <header class="site-header site-header--home-video" aria-label="{esc(landing['aria'])}">
    ...
    <a class="site-header__lang site-header__lang--corner" href="{lang_href}" hreflang="{other}" lang="{other}" aria-label="{esc(lang_aria)}">{other_label}</a>
...
{hero_preload}  <link rel="preload" href="/assets/css/site.min.css?v={SITE_CSS_VERSION}" as="style">
  <link rel="stylesheet" href="/assets/css/site.min.css?v={SITE_CSS_VERSION}">
...
def render_home(lang: str) -> str:
    ...
    home_label = "Početna" if lang == "hr" else "Home"
    return f"""{head_meta(lang, home['title'], home['meta_description'], home['keywords'], canonical, is_home=True, og_image=home['image'], og_image_alt="Glavani Park adventure and zipline park in Istria, Croatia", body_class="theme-page home-landing", early_head=render_home_hero_preload(), extra_head=render_home_hero_critical_css())}
