...
def _human_catapult_snippets(lang: str) -> list[str]:
    ...
    return [
        ...
        "<p>Staff fit your harness, run a full safety briefing, and supervise every launch. You lie in the frame while the cables tension; you hold the release handle or the instructor controls the moment, depending on the setup. When the cables release, you feel acceleration quicker than an F1 car off the line — from zero to motorway speed in a single hit. Then you decelerate smoothly as the system brings you back. The whole process, including briefing and queue time, typically takes 15–20 minutes per group.</p>",
