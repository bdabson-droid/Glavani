...
(function () {
  ...
  function formatCalendarDayLabel(year, month, day) {
    ...
    const formatted = formatVisitDate(iso);
  ...
  function isWithin48Hours(dateIso) {
    if (!dateIso) return false;
    const visitDay = parseIsoDateLocal(dateIso);
    const todayStart = localToday();
    const dayDiff = Math.round((visitDay.getTime() - todayStart.getTime()) / 86400000);
    if (dayDiff < 0) return false;
    if (dayDiff <= 1) return true;
    const visitOpen = new Date(visitDay);
    visitOpen.setHours(9, 0, 0, 0);
    const diff = visitOpen.getTime() - Date.now();
  ...
  function buildMessage() {
    ...
    return [
      ...
      `${t.total}: €${total}`,
      `${t.date}: ${formatVisitDate(selectedDate)}`,
    ...
    return `<section class="book-panel">
      ...
      <div class="book-summary">
        ...
        <dl>
          ...
          <dt>${t.date}</dt><dd>${formatVisitDate(selectedDate)}</dd>
