 ...
 * Groups of more than 10 must call. Bookings submit via Cloudflare Static Forms.
...
(function () {
  ...
  const i18n = {
    en: {
      ...
      confirmLead: 'Check your package and total below, then send your booking request. We aim to get back to you within 48 hours.',
      ...
      submitSuccessFollowUp: 'We aim to get back to you within 48 hours. If you do not hear back, check your spam or junk folder for an email from info@glavanipark.com or office@glavanipark.com. If there is nothing there, we are a small family business and may have missed it — sorry! Please give us a call to confirm.',
    ...
    hr: {
      ...
      confirmLead: 'Provjerite paket i ukupnu cijenu u nastavku, zatim pošaljite zahtjev za rezervaciju. Nastojimo odgovoriti u roku od 48 sati.',
      ...
      submitSuccessFollowUp: 'Nastojimo odgovoriti u roku od 48 sati. Ako ne dobijete odgovor, provjerite neželjenu poštu za e-mail s info@glavanipark.com ili office@glavanipark.com. Ako ni tamo nema poruke, mala smo obiteljska tvrtka i možda smo propustili zahtjev — ispričavamo se! Molimo nazovite nas radi potvrde.',
  ...
  function buildFormData() {
    ...
    fd.set('static-form-name', 'glavani-booking');
    fd.set('lang', lang);
    fd.set('header', t.msgHeader);
    fd.set('package', a ? a.name : '');
    fd.set('date', selectedDate || '');
    fd.set('arrival', arrivalLabel());
    ...
    fd.set('total', `€${total}`);
    if (activityIsFamilyPackage(a)) {
      fd.set('guests', String(a.fixed_guests));
      fd.set('adults', '');
      fd.set('children', '');
    } else if (activityHasChildPrice(a)) {
      fd.set('guests', String(guestCount()));
      fd.set('adults', String(state.adults));
      fd.set('children', String(state.children));
    } else {
      fd.set('guests', String(guestCount()));
      fd.set('adults', '');
      fd.set('children', '');
  ...
  function showSuccessDialog() {
    ...
    if (!dialog) {
      ...
      dialog.innerHTML = `<div class="book-success-dialog__inner">
        ...
        <p class="book-success-dialog__followup"></p>
        <div class="book-success-dialog__actions">
          <a class="btn-primary book-success-dialog__call" href="#"></a>
        </div>
        <form method="dialog">
          <button type="submit" class="btn-secondary book-success-dialog__close"></button>
        </form>
      </div>`;
      document.body.appendChild(dialog);
    }
    dialog.querySelector('#book-success-dialog-title').textContent = t.submitSuccessTitle;
    dialog.querySelector('.book-success-dialog__lead').textContent = t.submitSuccessLead;
    dialog.querySelector('.book-success-dialog__followup').textContent = t.submitSuccessFollowUp;
    const callBtn = dialog.querySelector('.book-success-dialog__call');
    callBtn.textContent = `${t.submitSuccessCall} · ${lang === 'hr' ? '+385 98 224 314' : '+385 91 896 4525'}`;
    callBtn.href = `tel:+${phone}`;
    dialog.querySelector('.book-success-dialog__close').textContent = t.submitSuccessClose;
