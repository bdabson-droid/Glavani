/**
 * Contact enquiry form — Cloudflare Static Forms via fetch POST.
 */
(function () {
  ...
  const t = {
    en: {
      ...
      successSpam:
        'If you do not hear back, check spam for email from info@glavanipark.com, or call us to confirm.',
      error: 'Sorry, we could not send your message. Please call us instead.',
      linksNotAllowed:
        'Please remove any website links from your message and try again, or call us instead.',
    },
    hr: {
      submitting: 'Šaljem…',
      ...
      successSpam:
        'Ako ne dobijete odgovor, provjerite neželjenu poštu za info@glavanipark.com, ili nas nazovite.',
      error: 'Nažalost nismo mogli poslati poruku. Molimo nazovite nas.',
      linksNotAllowed:
        'Uklonite sve poveznice na web stranice iz poruke i pokušajte ponovno, ili nas nazovite.',
    },
  }[lang];
  ...
  function contactFieldsContainLink() {
    const name = form.querySelector('#contact-name')?.value.trim() || '';
    const message = form.querySelector('#contact-message')?.value.trim() || '';
    const activity = form.querySelector('#contact-activity')?.value.trim() || '';
    return (
      containsLink(message) ||
      containsLink(name) ||
      containsLink(activity)
    );
  }
  ...
  function showFeedback(html, isError) {
    ...
    feedback.className =
      ...
      (isError ? ' contact-form__feedback--error' : ' contact-form__feedback--success');
    feedback.innerHTML = html;
  }

  function buildFormData() {
    const fd = new FormData(form);
    fd.set('static-form-name', 'glavani-contact');
    return fd;
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (contactFieldsContainLink()) {
      showFeedback(`<p>${t.linksNotAllowed}</p>`, true);
      return;
    }
